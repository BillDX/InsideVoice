...
speak, release** — the transcript lands in whatever app has focus. For long
dictation, **tap Right ⌘ while holding** to lock recording on hands-free,
then tap Right ⌥ to finish. Everything runs on-device: no network at
runtime, audio never touches disk, transcripts are never logged.

## Requirements

- **Apple Silicon** Mac (M1 or later; built on an M3)
- **macOS 14 or later.** Built and tested on macOS 26. 14 (Sonoma) and 15
  (Sequoia) are supported but haven't been tried on real hardware yet, so a
  report from either is especially useful, even if it's just "it works".
...
4. **Keyboard check** — press your right-hand ⌥ and the diagram lights up
   green (it'll correct you if you hit the left one). The Right ⌘ next to
   it is marked "lock": hold ⌥, tap ⌘, and both light up orange
...
- **Hands-free lock** (latch): hold Right ⌥, tap Right ⌘, let go of both,
  talk for a while with pauses, tap Right ⌥ to finish — text should land at
  the cursor
